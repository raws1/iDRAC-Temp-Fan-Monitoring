...
function format_temp_f () {
    ...
    awk -v mode="$mode" -v value="$value" -v prefix="$prefix" '
        BEGIN {
            if (mode == "delta") {
                f = value * 9 / 5
            } else {
            ...
            if (f == int(f)) {
                printf "%s%d F", prefix, f
            } else {
                printf "%s%.1f F", prefix, f
            }
        }
    '
}

function format_setfanspeed_check_f () {
    local raw="$1"

    if [[ "$raw" =~ ^[+-]?[0-9]+$ ]]; then
        format_temp_f absolute "$raw"
    elif [[ "$raw" =~ ^([0-9]+)[[:space:]]*x[[:space:]]*([+-]?[0-9]+)$ ]]; then
...
function setfanspeed () { 
    ...
    if [[ $FS == "auto" ]]; then
        if [ "$Logtype" != 0 ] && [ "$4" -eq 0 ]; then
                echo "> $(format_setfanspeed_check_f "$TEMP_Check") is higher or equal to $(format_temp_f absolute "$TEMP_STEP"). Switching to automatic fan control"
        fi
        ...
        if [ "$4" -eq 1 ]; then
            ...
            echo "> $(format_setfanspeed_check_f "$TEMP_Check") is lower or equal to $(format_temp_f absolute "$TEMP_STEP"). Switching to manual $FS % control"
        fi
...
get_ipmi_temp_by_id () {
    ...
    echo "$DATADUMP" | grep "$sensor_id" | grep degrees | grep -oE '[0-9]{1,3}' | tail -1
}
...
if $NICPU_toggle ; then
    ...
    CPUTEMP0=$(get_ipmi_temp_by_id "${CPUID0:-}")
    CPUTEMP1=$(get_ipmi_temp_by_id "${CPUID1:-}")
    CPUTEMP2=$(get_ipmi_temp_by_id "${CPUID2:-}")
    CPUTEMP3=$(get_ipmi_temp_by_id "${CPUID3:-}")
fi
#CPU counting
if [ -z "$CPUTEMP0" ]; then
        CPUcount=0
...
AMBTEMP=$(get_ipmi_temp_by_id "${AMBIENT_ID:-}")
if [ $CPUcount != 0 ]; then
        if [[ ! -z "$AMBTEMP" ]]; then
                if $Logloop ; then
                ...
                if [ "$AMBTEMP" -ge $AMBTEMP_MAX ]; then
                        echo "Intake temp is very high!! : $(format_temp_f absolute "$AMBTEMP")!"
                        TEMPMOD=$MAX_MOD
                else
...
EXHTEMP=$(get_ipmi_temp_by_id "${EXHAUST_ID:-}")
if [ $CPUcount != 0 ]; then
        if [[ ! -z "$EXHTEMP" ]]; then
                if [ "$EXHTEMP" -ge $EXHTEMP_MAX ]; then
                        echo "Exhaust temp is critical!! : $(format_temp_f absolute "$EXHTEMP")!"
                        TEMPMOD=$MAX_MOD
                fi
        fi
else
        ...
        if [[ ! -z "$EXHTEMP" ]] && [[ ! -z "$AMBTEMP" ]]; then
                if $AMBDeltaMode ; then
                        if [ "$AMBTEMP" -ge "$EXHTEMP" ]; then
                                echo "!! Intake = $(format_temp_f absolute "$AMBTEMP") / Exhaust = $(format_temp_f absolute "$EXHTEMP") !!"
        ...
        if $AMBDeltaMode ; then
                ...
                if [ $vTEMP -ge $((MAXTEMP / DeltaR)) ]; then
                        ...
                        if [ $AMBloop_arg3 -gt $DAEloop_arg3 ]; then
                                ...
                                echo "Ambient temperature ($(format_temp_f absolute "$AMBloop_arg1")) requires higher cooling than Delta A/E profile."
